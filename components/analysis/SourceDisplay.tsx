...
export default function SourceDisplay({ darkMode = false, toggleDarkMode, renderHighlights = true, fontSize = 16 }: SourceDisplayProps) {
  ...
  const markdownComponents = useMemo<Components>(() => ({
    ...
    blockquote: ({node, ...props}) => <blockquote style={{fontSize: `${fontSize}px`}} className="border-l-4 pl-5 py-2 my-5 italic font-['Lora',serif] dark:border-indigo-500 border-indigo-300 dark:bg-indigo-900/20 bg-indigo-50/50 dark:text-indigo-200 text-indigo-800" {...props} />,
    ...
    code: ({ className, children, ...props }: any) => {
      ...
      if (props.inline) {
        return (
          <code style={{fontSize: `${fontSize * 0.875}px`}} className="px-1.5 py-0.5 rounded font-[var(--font-geist-mono)] bg-slate-200 text-purple-700 dark:bg-slate-700 dark:text-amber-300" {...props}>
            {children}
          </code>
        );
      }
    ...
    pre: ({node, children, ...props}) => (
      <div className="my-6">
        <pre 
          ...
          className={`p-4 rounded-md overflow-x-auto font-[var(--font-geist-mono)] shadow-inner ${
    ...
    td: ({node, ...props}) => <td style={{fontSize: `${fontSize * 0.875}px`}} className="px-4 py-2 font-['Lora',serif]" {...props} />,
  ...
  const renderParagraphWithHighlights = (paragraph: string, paragraphIndex: number) => {
    ...
    if (paragraph.trim().startsWith('```')) {
      return (
        <div key={paragraphIndex} className="mb-4">
          <pre style={{fontSize: `${fontSize * 0.875}px`}} className={`p-4 rounded-md overflow-x-auto font-[var(--font-geist-mono)] shadow-inner ${
            darkMode ? 'bg-slate-800 text-slate-200 ring-1 ring-slate-700' : 'bg-slate-100 text-slate-800 ring-1 ring-slate-200'
          }`}>
            {paragraph.replace(/^```(\w*)\n/, '').replace(/```$/, '')}
          </pre>
    ...
    if (relevantSegments.length === 0) {
      ...
      return <p key={paragraphIndex} style={{fontSize: `${fontSize}px`}} className="mb-4 font-['Lora',serif]">{paragraph}</p>;
    }
    ...
    return <p key={paragraphIndex} style={{fontSize: `${fontSize}px`}} className="mb-4 font-['Lora',serif]">{parts}</p>;
  };
  ...
  {sourceType === 'image' && <div className="relative max-h-[40vh] my-0 rounded overflow-hidden "><Image src={URL.createObjectURL(sourceFile)} alt="Source document image" className="object-contain" fill /></div>}
          ...
          {showHighlights ? (
            <div className={`font-['Lora',serif] leading-relaxed ${darkMode ? 'text-slate-200' : 'text-slate-800'}`}>
  ...
  return (
    <div className="relative">
      ...
      {isHighlightMode && highlightedSegments && highlightedSegments.length > 0 && renderHighlights && (
        <div className="absolute -top-10  left-1/2 transform -translate-x-1/2 z-10 transition-all duration-300 ease-out">
          <div className="bg-yellow-100 border border-yellow-300 rounded-full px-3 py-1 text-xs text-yellow-900 shadow-md flex items-center space-x-1.5 backdrop-blur-sm">
      ...
      <div ref={containerRef}
        className={`overflow-y-auto border ${darkMode ? 'border-slate-700 bg-slate-900/95' : 'border-slate-300 bg-white'} shadow rounded-md transition-colors duration-300`}
      ...
      {/* Resizer handle */}
      <div className={`absolute rounded-lg bottom-0 left-0 right-0 h-5 ${darkMode ? 'bg-gradient-to-t from-slate-800/80 to-transparent' : 'bg-gradient-to-t from-slate-100/80 to-transparent'} cursor-ns-resize flex justify-center items-end transition-colors duration-300 group`}
