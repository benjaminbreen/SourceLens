...
export default async function handler(
  ...
  try {
    ...
    - date: the most likely publication date found in ISO format (YYYY-MM-DD) or provide your best guess if unclear from text. ALWAYS provide a guess here. This must be date CREATED, not other dates that appear in text.
    - author: the most likely author name or say UNKNOWN if truly uncertain. In some cases there will be an AUTHOR and an EDITOR, in which case list the author here and editor in additionalInfo. If a first or last name is initials, silently expand to your best guess (i.e. change W. to William, or whatever makes most sense based on context.)
